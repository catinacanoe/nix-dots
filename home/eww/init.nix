...
function change_mus_color() {
    ...
    text='bg'
    if [ "$(echo "$newcol" | grep 'bg\|t0\|t1\|t2' | wc -l)" -gt "$(echo "$newcol" | grep 'fg\|t7\|t6' | wc -l)" ]; then
        text='fg'
    fi

    hashes="$(
    while IFS= read -r line; do
        hash="$(get_col_rgb "$line")"
        [ "$hash" == "ERROR" ] && return 1
        echo -n "$hash, "
    done <<< "$newcol"
    echo
    ...
    texthash="$(get_col_rgb "$text")"

    hashes="$(echo "$hashes" | sed 's|, $||' )"

    newcss="$(
    cat "$XDG_CONFIG_HOME/eww/eww.scss.gen"
    echo "/* AUTOGENERATED from eww/init.sh */"
    echo ".mus-gradient {"
    echo "    background-image: linear-gradient(115deg, $hashes);"
    echo "    color: $texthash;"
    echo "}"
    echo ".left {"
    echo "    background: $texthash;"
    echo "}"
    )"
...
function music() {
    ...
    while true; do
        ...
        [ -n "$(cat /tmp/eww-test-color)" ] && color="$(cat /tmp/eww-test-color)"

        if [ "$old_color" != "$color" ]; then
            old_color="$color"
            change_mus_color "$color"
        fi
