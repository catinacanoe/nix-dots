...
in
{
    ...
    if [ -z "$1" ]; then
        echo "please pass an action: 'set' 'gen'"
        exit
    fi
    ...
    function convertimages() {
        ...
        echo "generation 1/8 image with ${color} overlay"
        convert "$target" -fill '${color}' -colorize 65% -quality 100 -depth 16 "$DIM"

        echo "generation 2/8 reflecting it vertically"
        convert "$DIM" -flip -quality 100 -depth 16 "$DIMVFLIP"
        echo "generation 3/8 joining the vertical images"
        convert "$DIM" "$DIMVFLIP" -append -quality 100 -depth 16 "$DIMVFULL"
        echo "generation 4/8 reflecting horizontally"
        convert "$DIMVFULL" -flop -quality 100 -depth 16 "$DIMHFLIP"
        echo "generation 5/8 joining the images"
        convert "$DIMVFULL" "$DIMHFLIP" +append -quality 100 -depth 16 "$DIMFULL"

        echo "generation 6/8 blurring 4 directional image"
        convert "$DIMFULL" -blur 0x30 -quality 100 -depth 16 "$DIMBLUR"

        echo "generation 7/8 blurring original image"
        convert "$target" -blur 0x30 -quality 100 -depth 16 "$BLUR"

        echo "generation 8/8 compressing original image"
    ...
    function writenix() {
        echo "# $target" > "$NIXFILE"
        echo "{ # AUTOGENERATED BY WPP SCRIPT" >> "$NIXFILE"
